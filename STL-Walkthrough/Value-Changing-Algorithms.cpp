...
* Value-changing Algorithms - Changes the element values
...
transform(vec.begin(), vec.end(),				// source #1	
		...
		// Add items from vec nd vec2 and save in vec3
		// vec3[0] = vec[0] + vec2[0] 
		// vec3[0] = vec[1] + vec2[1]
		// ...
...
replace_if(vec.begin(), vec.end()		// source
			[](int x){return x>80;},qqq// Old value condition
			9);							// new vallue
...
replace_copy(vec.begin(), vec.end(),	// SOurce
 ...
 // Generalizzed form : replace_copy_if()
...
remove_copy(vec.begin(), vec.end(),  // Source
	...
	// .. Removes all 6's and copy items to vec2 
  	// Genralized form : remove_copy_if()
...
unique_copy(vec.begin(), vec.end(),vec2.begin())
	// Remove consecutive equal elements , and then copu the uniquified items to vec2
...
*	They changes the order of elements in container , but not necessarily 
*	the elements themselves.	
*
*/


vector<int> vec = {9, 60, 7, 8, 45, 87, 90};		// 7 items 
...
prev_permutation(vec.begin(), vec.end());
			// :lexicogaphically next smaller permutation 

// {1,2,3,5} < {1,2,4,4}	
// {1,2}	< {1,2,3}	

// Sorted in asc. order : {8,9,45,60,70,87,90}
//						   -  lexicogaphically smallest
//
// Sorted in desc. order : {90,87,70,60,45,9,8}
...
// 	 (swap each element with a randomly selected elements)
